...
/// A solution contains one or many <see cref="ArgProject"/>s.
/// </summary>
public class ArgSolution : IFileHandle
{
    /// <summary>
    ...
    /// </summary>
    public List<ArgProject> Projects { get; }

    public ArgSolution(string filename)
    {
        (Directory, Name) = filename.GetDirectoryAndName();

        Projects = new List<ArgProject>();
    }
    ...
    /// <returns>New instance of <see cref="ArgSolution"/> containing the read information.</returns>
    public static ArgSolution ReadSolution(string filename)
    {
        ArgSolution solution = new ArgSolution(filename);
        ...
        if (version >= Version.AddedReferencedProjects)
        {
            ...
            for (int i = 0; i < projectsCount; i++)
            {
                ...
                {
                    solution.Projects.Add(ArgProject.ReadProject(projectFilename));
                }
            }
        }
    ...
    /// <summary>
    /// Creates a new <see cref="ArgSolution"/> and saves it to <paramref name="filename"/>.
    /// </summary>
    ...
    public static ArgSolution CreateAndSaveBlank(string filename)
    {
        ArgSolution solution = new ArgSolution(filename);
        solution.Save();
        return solution;
